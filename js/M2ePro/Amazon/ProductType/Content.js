window.AmazonProductTypeContent = Class.create(Common, {
    templates: {
        field: '',
        fieldset: '',
        image_option: '',
    ...
    initialize: function () {
        this.constFieldNotConfigured = M2ePro.php.constant(
            'Ess_M2ePro_Model_Amazon_Template_ProductType::FIELD_NOT_CONFIGURED'
        );

        this.constFieldCustomValue = M2ePro.php.constant(
            'Ess_M2ePro_Model_Amazon_Template_ProductType::FIELD_CUSTOM_VALUE'
        );

        this.constFieldCustomAttribute = M2ePro.php.constant(
            'Ess_M2ePro_Model_Amazon_Template_ProductType::FIELD_CUSTOM_ATTRIBUTE'
        );
        ...
        Validation.add(
            ...
            function (value, element) {
                ...
                if (value == AmazonProductTypeContentObj.constFieldNotConfigured) {
                    return false;
                }

                if (value == AmazonProductTypeContentObj.constFieldCustomValue) {
    ...
    renderFieldset: function (group, fieldset) {
        ...
        AmazonProductTypeTabsObj.addTabContent(group, fieldsetElement);
    ...
    initGroups: function (groups) {
        ...
        for (var i = 0; i < groups.length; i++) {
            AmazonProductTypeTabsObj.insertTab(groups[i]['nick'], groups[i]['title']);
        }

        AmazonProductTypeTabsObj.refreshTabs();
    },
