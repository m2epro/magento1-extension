    ...
    extends Ess_M2ePro_Controller_Adminhtml_Walmart_MainController
{
    ...
    public function indexAction()
    {
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            $block = $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_walmart_listing');
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_walmart_manageListings'))
            ->renderLayout();
    }
    ...
    public function getListingTabAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_walmart_listing')->toHtml()
        );
    }
    ...
    public function getListingOtherTabAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_walmart_listing_other')->toHtml()
        );
    }
    ...
    public function getSearchTabAction()
    {
        ...
        $this->getResponse()->setBody(
            $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_walmart_listing_search')->toHtml()
        );
    }
    ...
    protected function scheduleAction($action, array $params = array())
    {
        ...
        foreach ($listingsProducts as $index => $listingProduct) {
            if (in_array($action, array(Ess_M2ePro_Model_Listing_Product::ACTION_STOP,
        ...
        if (in_array($action, array(Ess_M2ePro_Model_Listing_Product::ACTION_STOP,
            ...
            foreach ($listingsProducts as $index => $listingProduct) {
                if ($action == Ess_M2ePro_Model_Listing_Product::ACTION_DELETE && !$listingProduct->isNotListed()) {
                    continue;
                }

                if ($action == Ess_M2ePro_Model_Listing_Product::ACTION_STOP && $listingProduct->isStoppable()) {
                    continue;
                }
        ...
        if ($action == Ess_M2ePro_Model_Listing_Product::ACTION_REVISE) {
            ...
            $tag = '/qty/price/promotions/details/';
        ...
        foreach ($listingsProducts as $listingProduct) {
            ...
            $item = $existedScheduledActionsCollection->getItemByColumnValue(
                'listing_product_id', $listingProduct->getId()
            );
            if ($item) {
    ...
    {
        return $this->getResponse()->setBody($this->scheduleAction(
    ...
    public function runResetProductsAction()
    {
        ...
        /** @var Ess_M2ePro_Model_Mysql4_Listing_Product_Collection $listingsProducts */
    ...
    public function mapToTemplateCategoryAction()
    {
        ...
        /** @var Ess_M2ePro_Model_Walmart_Template_Category $template */
        $template = Mage::getModel('M2ePro/Walmart_Template_Category')->load($templateId);
        ...
        $diff->setNewSnapshot($snapshotBuilder->getSnapshot());

        $listingsProductsCollection = Mage::getResourceModel('M2ePro/Listing_Product_Collection');
        $listingsProductsCollection->addFieldToFilter('id', $productsIds);

        $changeProcessor = Mage::getModel('M2ePro/Walmart_Template_Category_ChangeProcessor');
        $changeProcessor->process($diff, $listingsProductsCollection->getData());
            ...
            $messages[] = array(
                ...
                'text' => '<p>' . Mage::helper('M2ePro')->__(
                    'Category Policy cannot be unassigned from some Products because they are
                     participating in the new Product Type(s) creation.') . '</p>'
            );
        } else {
                ...
                $messages[] = array(
                    ...
                    'text' => '<p>' . Mage::helper('M2ePro')->__(
                        'Category Policy cannot be unassigned because the Products are in Action or
                         in the process of new Product Type(s) Creation.'). '</p>'
                );
            }
        }
    ...
    private function processDescriptionTemplateChange(array $oldData,
                                                      array $newData,
    ...
    private function processSellingFormatTemplateChange(array $oldData,
                                                        array $newData,
                                                        array $affectedListingsProductsData)
    {
        if (empty($affectedListingsProductsData) ||
    ...
    private function processSynchronizationTemplateChange(array $oldData,
                                                          array $newData,
                                                          array $affectedListingsProductsData)
    {
        if (empty($affectedListingsProductsData) ||
