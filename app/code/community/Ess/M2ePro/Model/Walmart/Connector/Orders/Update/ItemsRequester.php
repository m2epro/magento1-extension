...
abstract class Ess_M2ePro_Model_Walmart_Connector_Orders_Update_ItemsRequester
    extends Ess_M2ePro_Model_Walmart_Connector_Command_Pending_Requester
{
    ...
    {
        return 'Walmart_Connector_Orders_Update_ProcessingRunner';
    }
    ...
    public function getRequestData()
    {
        ...
        $request = array(
            'id'               => $order['change_id'],
            'order_id'         => $order['walmart_order_id'],
        ...
        if (isset($order['items']) && is_array($order['items'])) {
            ...
            foreach ($order['items'] as $item) {
                $request['items'][] = array(
                    'item_code' => $item['walmart_order_item_id'],
                    'qty'       => (int)$item['qty']
                );
            }
        }
