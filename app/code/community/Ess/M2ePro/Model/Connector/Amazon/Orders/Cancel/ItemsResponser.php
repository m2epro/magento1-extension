...
class Ess_M2ePro_Model_Connector_Amazon_Orders_Cancel_ItemsResponser
    extends Ess_M2ePro_Model_Connector_Amazon_Responser
{
    // M2ePro_TRANSLATIONS
    // Amazon Order was not cancelled. Reason: %msg%
    // Amazon Order was cancelled.
    ...
    public function unsetProcessingLocks(Ess_M2ePro_Model_Processing_Request $processingRequest)
    {
        ...
        foreach ($this->getOrders() as $order) {
            $order->deleteObjectLocks('cancel_order', $processingRequest->getHash());
        }
    }
    ...
    public function eventFailedExecuting($message)
    {
        ...
        foreach ($this->getOrders() as $order) {
            ...
            $order->addErrorLog('Amazon Order was not cancelled. Reason: %msg%', array('msg' => $message));
        }
    }
    ...
    protected function processResponseData($response)
    {
        ...
        if (count($globalMessages) > 0) {
            foreach ($orders as $order) {
                foreach ($globalMessages as $message) {
                    ...
                    $order->getLog()->setInitiator(Ess_M2ePro_Helper_Data::INITIATOR_EXTENSION);
                    $order->addErrorLog('Amazon Order was not cancelled. Reason: %msg%', array('msg' => $text));
                }
            }
        ...
        foreach ($response['messages'] as $changeId => $messages) {
            ...
            foreach ($messages as $message) {
                ...
                $orders[$orderId]->addErrorLog('Amazon Order was not cancelled. Reason: %msg%', array('msg' => $text));
            }
        }
        // ---------------------------------------
        ...
        foreach ($this->params as $changeId => $requestData) {
            ...
            $orders[$orderId]->getLog()->setInitiator(Ess_M2ePro_Helper_Data::INITIATOR_EXTENSION);
            $orders[$orderId]->addSuccessLog('Amazon Order was cancelled.');
        }
    }
