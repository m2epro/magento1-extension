...
class Ess_M2ePro_Model_Order_Note extends Ess_M2ePro_Model_Abstract
{
    ...
    protected function _afterSave()
    {
        ...
        if (!is_null($this->getOrigData('id'))) {
            $comment = Mage::helper('M2ePro')->__(
    ...
    private function updateMagentoOrderComments($comment)
    {
        $magentoOrderModel = $this->getOrder()->getMagentoOrder();

        if (!is_null($magentoOrderModel)) {
    ...
    public function getOrder()
    {
        if (is_null($this->order)) {
            $this->order = Mage::helper('M2ePro/Component')->getUnknownObject('Order', $this->getOrderId());
        }
