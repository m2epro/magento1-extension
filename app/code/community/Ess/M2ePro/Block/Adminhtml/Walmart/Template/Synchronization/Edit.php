    ...
    public function __construct()
    {
        ...
        if (!Mage::helper('M2ePro/Component')->isSingleActiveComponent()) {
            ...
            if ($this->isEditMode()) {
                ...
                $this->_headerText = Mage::helper('M2ePro')->__('Add %component_name% Synchronization Policy',
                    $componentName
                );
            }
        } else {
            if ($this->isEditMode()) {
                $this->_headerText = Mage::helper('M2ePro')->__('Edit Synchronization Policy "%template_title%"',
                    $this->escapeHtml(Mage::helper('M2ePro/Data_Global')->getValue('temp_data')->getTitle())
                );
            } else {
